...
DECLARE
  ...
  v_added_by_type TEXT := 'host';
  v_added_by_attendee_profile_id UUID := NULL;
BEGIN
  ...
  IF v_linked_profile_id IS NOT NULL THEN
    -- Preserve the entered guest name while attributing ownership to the linked profile.
    -- This mirrors the existing proxy-attendee semantics used elsewhere in the app.
    v_added_by_type := 'proxy';
    v_added_by_attendee_profile_id := v_linked_profile_id;
  END IF;
  ...
  IF v_existing_cancelled_id IS NOT NULL THEN
    ...
    SET
      ...
      added_by_type = v_added_by_type,
      added_by_attendee_profile_id = v_added_by_attendee_profile_id,
      joined_at = now(),
      promoted_at = NULL,
      cancelled_at = NULL
  ...
  ELSE
    ...
    VALUES (
      ...
      v_status,
      v_added_by_type,
      v_added_by_attendee_profile_id
    )
    RETURNING id INTO v_attendee_id;
  END IF;
