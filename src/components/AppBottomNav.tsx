...
export function AppBottomNav({ user }: AppBottomNavProps) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-2xl items-stretch justify-between px-3 pb-[max(env(safe-area-inset-bottom),0.2rem)] pt-0.5">
        {items.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  'flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-2xl px-2 py-1 text-[10px] font-bold transition-colors',
                  isCreateItem
                    ? 'text-slate-500'
                    : isActive
                      ? 'text-brand-700'
              ...
              {({ isActive }) => (
                <>
                  {isCreateItem ? (
                    <div
                      className={cn(
                        'relative -mt-3.5 flex h-11 w-11 items-center justify-center rounded-[1.15rem] shadow-[0_8px_18px_rgba(13,148,136,0.26)] ring-1 ring-white/80 transition-all',
                        isActive
                    ...
                    >
                      <span className="pointer-events-none absolute inset-0 rounded-[1.15rem] bg-gradient-to-b from-white/30 via-white/10 to-transparent" />
                      <span
                        className="relative h-7 w-7 bg-white"
                        style={{
                    ...
                    <div
                      className={cn(
                        'flex h-8 w-8 items-center justify-center rounded-[1.15rem] transition-colors',
