...
-- -------------------------------------------------------------------
-- 6) Custom join field support (host-only answer visibility)
-- -------------------------------------------------------------------
...
DO $$
BEGIN
    IF to_regclass('public.event_signup_field_answers') IS NOT NULL AND NOT EXISTS (
        SELECT 1
        FROM pg_trigger
        WHERE tgname = 'event_signup_field_answers_touch_updated_at'
          AND tgrelid = to_regclass('public.event_signup_field_answers')
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
          ...
          AND policyname = 'Hosts can view custom signup answers'
    ) THEN
...
-- -------------------------------------------------------------------
-- Gallery feature reconciliation
-- -------------------------------------------------------------------
ALTER TABLE public.events
...
-- -------------------------------------------------------------------
-- 6) Runtime moderation policy controls
-- -------------------------------------------------------------------
